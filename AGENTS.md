...
- **`src/api/client.ts`** — single HTTP boundary. `apiFetch()` auto-attaches `Authorization: Bearer ${RELEASED_API_KEY}` when admin mode is active, and (via `src/lib/mutation-log.ts`) records mutating requests when `RELEASES_RUN_DIR` is set.
