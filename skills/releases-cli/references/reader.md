...
`publish-token create` requires a verified domain ownership claim on the source's org (see the [GitHub Actions integration doc](https://releases.sh/docs/integrations/github-actions)); it mints a `relk_` token scoped to ONE source, printing only the token to stdout (so it pipes straight into `gh secret set`) with setup guidance on stderr. Like every `publish-token`/`keys` verb, it opens its own one-time browser approval and signs the session back out once the command finishes — there's no persisted login session to reuse.
