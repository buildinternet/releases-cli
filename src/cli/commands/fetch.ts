  ...
  releases admin source fetch --org acme            Fetch all active sources for an org`)
    .action(async (slugArg: string | undefined, opts: {
      ...
      if (opts.org) {
        ...
        sourceIdentifiers = activeSources.map((s) => s.id);
        label = `all sources for ${org.slug} (${sourceIdentifiers.length})`;
      } else if (slug) {
        if (slug.startsWith("src_")) {
          sourceIdentifiers = [slug];
        } else {
          const src = await findSource(slug);
          if (!src) return sourceNotFound(slug);
          sourceIdentifiers = [src.id];
      ...
      try {
        ...
        const overlapping = sourceIdentifiers.filter((id) => activeSlugs.includes(id));
        if (overlapping.length > 0) {
          const overlapSessionId = sessionMap[overlapping[0]];
          const sourceList = overlapping.length <= 3 ? overlapping.join(", ") : `${overlapping.length} sources`;
